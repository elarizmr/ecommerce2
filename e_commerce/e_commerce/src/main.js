...
if (slider) {
  ...
  next?.addEventListener("click", () => { index = (index + 1) % total; showSlide(index); });
  prev?.addEventListener("click", () => { index = (index - 1 + total) % total; showSlide(index); });
...
const iconHeart = (outline = true) => outline ? `<i class="ri-heart-line text-lg"></i>` : `<i class="ri-heart-fill text-lg text-red-500"></i>`;
const iconCart = () => `<i class="ri-shopping-cart-line"></i>`;
const iconScale = () => `<i class="ri-scales-3-line"></i>`;
...
function renderProducts() {
  grid.innerHTML = products.map((p) => `
    <article class="bg-white rounded-2xl border shadow-sm hover:shadow-md transition p-4 relative flex flex-col">
      <span class="absolute -left-2 -top-2 bg-rose-600 text-white text-xs font-bold px-2 py-1 rounded-lg shadow">- ${p.discount} ₼</span>
    ...
    <div class="flex items-center justify-between border-b pb-2">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-gray-500">${fmtMoney(item.price)}</p>
        </div>
      </div>
